...
c
     ...
     $               tm2  (LX1,LY1,LZ1,LELV),
     $               tm3  (LX1,LY1,LZ1,LELV)
 
      ntot1 = lx1*ly1*lz1*nelv

      !Calculate Divergence norms of new VX,VY,VZ
      call col3    (tm1,vx,vtrans,ntot1)
      call col3    (tm2,vy,vtrans,ntot1)
      call col3    (tm3,vz,vtrans,ntot1)

      CALL OPDIV   (DVC,tm1,tm2,tm3)
      CALL DSSUM   (DVC,lx1,ly1,lz1)
      CALL COL2    (DVC,BINVM1,NTOT1)
      ...
      IF (NIO.EQ.0) THEN
         WRITE(6,'(13X,A,1p2e13.4)')
     &         'L1/L2 DIV(rhosV)    ',DIV1,DIV2
         WRITE(6,'(13X,A,1p2e13.4)') 
     &         'L1/L2 QTL           ',QTL1,QTL2
         WRITE(6,'(13X,A,1p2e13.4)')
     &         'L1/L2 DIV(rhosV)-QTL',DIF1,DIF2
         IF (DIF2.GT.0.1) WRITE(6,'(13X,A)') 
     &         'WARNING: DIV(rhosV)-QTL too large!'
      ENDIF
...
c-----------------------------------------------------------------------
      subroutine crespsp_anlst (respr)
...
c compute stress tensor for ifstrs formulation - variable viscosity Pn-Pn
     ...
     &                    ur,us,ut,vr,vs,vt,wr,ws,wt)
         ...
         call opcolv   (ta1,ta2,ta3,QvHinv)
         scale2 = -2./3. 
         call opadd2cm (w1,w2,w3,ta1,ta2,ta3,scale2)
         call opsub2   (wa1,wa2,wa3,w1,w2,w3)
...
c     add explicit (NONLINEAR) terms 
      n = lx1*ly1*lz1*nelv
      do i=1,n
         ta1(i,1) = bfx(i,1,1,1)-wa1(i)
         ta2(i,1) = bfy(i,1,1,1)-wa2(i)
         ta3(i,1) = bfz(i,1,1,1)-wa3(i)
      enddo
      call opdssum (ta1,ta2,ta3)
      do i=1,n
         ta1(i,1) = ta1(i,1)*binvm1(i,1,1,1)
...
C     add thermal divergence
      dtbd = BD(1)/DT
      call admcol4(respr,QTL,bm1,vtrans,dtbd,ntot1)
...
C     surface terms
     ...
     $      CALL ADD2   (W1(1,IEL),W3(1,IEL),NXYZ1)
            ...
            IF (CB(1:1).EQ.'V'.OR.CB(1:1).EQ.'v'.or.
     $         cb.eq.'MV '.or.cb.eq.'mv ') then
              CALL COL2C(W1(1,IEL),vtrans(1,1,1,IEL,1),dtbd,NXYZ1)
            endif
            CALL SUB2 (RESPR(1,IEL),W1(1,IEL),NXYZ1)
  300    CONTINUE
  100 CONTINUE
...
C     (only if all Dirichlet b.c.)
      ...
      END
c----------------------------------------------------------------------
      subroutine cresvsp_anlst (resv1,resv2,resv3,h1,h2)
...
C     Compute the residual for the velocity
     ...
     $ ,             TA4   (LX1,LY1,LZ1,LELV)
      ...
      call col3    (ta4,vdiff,QvHinv,ntot)
      call cmult   (ta4,scale,       ntot)
      call opgrad  (ta1,ta2,ta3,TA4)
      call opsub2  (resv1,resv2,resv3,ta1,ta2,ta3)
c
      call opgrad  (ta1,ta2,ta3,    pr)
      call opcolv  (ta1,ta2,ta3,vtrans)
      call opsub2  (resv1,resv2,resv3,ta1,ta2,ta3)

      call opadd2  (resv1,resv2,resv3,bfx,bfy,bfz)
      if(IFAXIS) then
         call col2 (resv2, omask,ntot)
         call col2 (resv3, omask,ntot)
      endif
...
c----------------------------------------------------------------------
      subroutine admcol4(a,b,c,d,e,n)
      REAL A(1),B(1),C(1),D(1),E
C
      DO 100 I=1,N
